...
class LogisticsWorker:
    ...
    async def investigate(self, case_id: str, order_id: str) -> LogisticsResult:
        ...
        # For a delivered order, complete timeline requires carrier_dt, customer_dt, estimated_dt, and shipping_limit
        ...
        # Schema enum: on_time, seller_delay, logistics_delay, lost, returned, conflicting, insufficient_evidence
        verdict: str
        suggested_issue: str | None = None
        responsible_party: dict[str, str | None] | None = None

        # Check for lost / returned states
        has_lost_event = any(
        ...
        if has_lost_event or order_status in ("lost", "canceled_in_transit"):
            ...
            if is_customer_late:
                if late_seller_ids:
                    # Seller handed over late to carrier, causing customer delivery delay
                    ...
                    # Seller handed over on time, carrier delayed delivery to customer
                    verdict = "logistics_delay"
                    suggested_issue = "late_delivery_logistics"
                    responsible_party = {"party_type": "logistics_provider", "party_id": None}
            else:
                # Customer received on or before estimated delivery date
                verdict = "on_time"
                suggested_issue = None
